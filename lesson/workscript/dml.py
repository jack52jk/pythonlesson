...
def main():
    preexetime=datetime.now().timestamp()
    count = 0
    while True:
        id=5000
        sql = "update batch set id=id-3000000 where id >  {} limit 10;".format(id)
        res,ok = update(sql)
        if not ok:
            print(res)
            break
            ...
            print("sql sucess")
        count=count+1
    print("sql exe count is :{}".format(count))

    afterexetime=datetime.now().timestamp()
    exetime=int(afterexetime-preexetime)
    
    print("operate execute time is : {}s".format(exetime))
